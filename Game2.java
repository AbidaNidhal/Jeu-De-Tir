...
public class Game2 {
    ...
    public Game2()
    {
        ...
        Thread threadForInitGame = new Thread() {
            ...
            public void run(){
                Initialize();
                LoadContent();
                Framework.gameState = Framework.GameState.PLAYING2;
            }
        };
    ...
    private void LoadContent()
    {
        try
        {
            ...
            URL TargetImgUrl = this.getClass().getResource("/jeu_de_tir/resources/images/target1.png");
    ...
    public void UpdateGame(long gameTime, Point mousePosition)
    {
        ...
        {boolean hit=false;
            if(System.nanoTime() - lastTimeShoot >= timeBetweenShots)
            {
                for(int i = 0; i < Targets.size(); i++)
                {
                    if(new Ellipse2D.Double(Targets.get(i).x,Targets.get(i).y,32,32).contains(mousePosition))
        ...
        if(killedTargets == 10) {
            ...
            scorefix=score;
            try {
                if (ScoreBoard.IsEmpty("C://niv3.txt") == true)
                ScoreBoard.reset("C://niv3.txt");
                } catch (Exception e1) {
                e1.printStackTrace();
            }
            ScoreBoard.AddtoScoreList("1/  " + Long.toString(score)+ "\n","C://niv3.txt");
            ScoreBoard.sortScoreboard("C://niv3.txt");
        }
    }
